...
var AugmentedEvent = class {
  ...
  update(e) {
    ...
    if (globalVars_default.enableScrollInfo && (mainType === "scroll" || mainType === "touchmove")) {
      const top = docEl2.scrollTop + document.body.scrollTop;
...
function rAFThrottle(func) {
  let throttle2 = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 15;
  let context;
  let args;
  let last = 0;
  let requestId = 0;
  const later = function() {
    const now = getTime();
    const remaining = throttle2 - (now - last);
    if (remaining <= 0) {
      last = now;
      requestId = 0;
...
function connectContinuousEvent(target, mainEvent, event) {
  return function throttleEvent(throttleRate, cb, options, eventOptions) {
    ...
    const ae = {
      start: new AugmentedEvent_default({
        mainType: mainEvent,
        subType: "start"
      }),
      // start
      main: new AugmentedEvent_default({
        mainType: mainEvent
      }),
      // main
      end: new AugmentedEvent_default({
        mainType: mainEvent,
        subType: "end"
      })
    };
...
function connectDiscreteEvent(target, event) {
  return function throttleEvent(throttleRate, cb, options, eventOptions) {
    ...
    const ae = new AugmentedEvent_default({
      mainType: event
    });
    function handler(e) {
      ae.update(e);
...
var win2;
var winHeight = -1;
var Sticky = class extends import_react.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const arePropsChanged = !(0, import_shallowequal.default)(this.props, prevProps);
    if (arePropsChanged) {
  ...
  render() {
    ...
    const outerClasses = (0, import_classnames.default)("sticky-outer-wrapper", this.props.className, {
      [this.props.activeClass]: this.state.status === STATUS_FIXED,
      [this.props.releasedClass]: this.state.status === STATUS_RELEASED
    });
    const innerClasses = (0, import_classnames.default)("sticky-inner-wrapper", this.props.innerClass, {
