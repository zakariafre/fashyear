...
var _AugmentedEvent = _interopRequireDefault(require("./AugmentedEvent"));
var _globalVars = _interopRequireDefault(require("./globalVars"));
var _listen = _interopRequireDefault(require("./lib/listen"));
var _rAFThrottle = _interopRequireDefault(require("./lib/rAFThrottle"));
...
function connectContinuousEvent(target, mainEvent, event) {
  return function throttleEvent(throttleRate, cb, options, eventOptions) {
    ...
    const ae = {
      start: new _AugmentedEvent.default({
        mainType: mainEvent,
        subType: 'start'
      }),
      // start
      main: new _AugmentedEvent.default({
        mainType: mainEvent
      }),
      // main
      end: new _AugmentedEvent.default({
        mainType: mainEvent,
        subType: 'end'
      })
    };
    ...
    if (throttleRate === 'raf') {
      ...
      handler = (0, _rAFThrottle.default)(handler);
    ...
    listeners[throttledMainEvent] = (0, _listen.default)(domTarget || target, mainEvent, handler, eventOptions);
    return remover;
  };
}
function connectDiscreteEvent(target, event) {
  return function throttleEvent(throttleRate, cb, options, eventOptions) {
    ...
    const ae = new _AugmentedEvent.default({
      mainType: event
    });
    function handler(e) {
      ae.update(e);
    ...
    listeners[throttledEvent] = (0, _listen.default)(domTarget || target, event, handler, eventOptions);
    return remover;
  };
}
